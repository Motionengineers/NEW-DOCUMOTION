...
function extractState(location) {
  ...
  const parts = location.split(',').map(part => normalise(part)).filter(Boolean);
  if (parts.length === 0) return null;
  // Return the last part, assuming it represents the State in "City, State"
...
function normaliseProfile(input = {}) {
  ...
  const rawSector = Array.isArray(input.sector) ? input.sector.join(', ') : (input.sector || '');
  const sectorTokens = normalise(rawSector)
    .split(/[\/,&]/)
