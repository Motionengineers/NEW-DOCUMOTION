...
export async function writeJson(fileName, value) {
  try {
    ...
    // On serverless platforms like Vercel, file system might be read-only
    // Log the error but don't fail - allow client-side storage to handle it
    if (error.code === 'EACCES' || error.code === 'EROFS' || error.code === 'ENOSPC') {
      console.warn(`writeJson: Cannot write to ${fileName} (read-only filesystem or no space):`, error.message);
      throw new Error(`File system is read-only or unavailable. Settings will be stored in browser only.`);
    }
    throw error;
  }
}
