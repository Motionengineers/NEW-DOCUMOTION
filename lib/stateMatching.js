...
function filterSchemesForProfile(profile, schemes = []) {
  ...
  return schemes.filter(scheme => {
    ...
    const stageMatch = !stage || !scheme.tags ? true : scheme.tags.toLowerCase().includes(stage);
...
export function computeStateMatches(profile, states, schemes, options = {}) {
  ...
  const candidateSchemes =
    options.prefilter === false ? schemes : filterSchemesForProfile(profile, schemes);
    ...
    .map(state => {
      ...
      for (const scheme of list) {
        sectorScore = Math.max(
          sectorScore,
          sectorMatchWeight(
            scheme.sector || scheme.subSector || '',
      ...
      const successEntry =
        historySuccess[state.id] || historySuccess[state.name?.toLowerCase() || ''] || {};
      const successScore =
...
export { computeStateMatches as default };
