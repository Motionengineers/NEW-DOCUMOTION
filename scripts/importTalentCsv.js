...
function inferSkills(title = '') {
  const t = title.toLowerCase();
  const set = new Set();
  if (/(ceo|founder|co-founder|cofounder|director|vp|head)/.test(t)) {
    set.add('leadership');
    set.add('strategy');
    set.add('operations');
  }
  if (/(cmo|marketing|growth|brand)/.test(t)) {
    set.add('marketing');
    set.add('branding');
    set.add('go-to-market');
  }
  if (/(cto|engineering|tech|developer|software|product)/.test(t)) {
    set.add('engineering');
    set.add('product');
    set.add('cloud');
  }
  ...
  return prisma.$transaction(
    async tx => {
      ...
      for (const r of rows) {
        ...
        const email = handle
          ? `${handle}@placeholder.email`
          : `${first}.${(last || '').split(' ')[0] || 'x'}@placeholder.email`.toLowerCase();
        ...
        let existing = await tx.talent.findFirst({
          where: {
            OR: [
              { email },
              {
                AND: [{ fullName: `${first} ${last}`.trim() }, { industry: company || undefined }],
              },
            ],
          },
        });
...
async function main() {
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    console.log(
      `Processed ${i + batch.length}/${rows.length} (+${inserted} new, ~${updated} updated)`
    );
    await sleep(50);
  }
