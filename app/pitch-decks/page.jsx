...
export default async function PitchDeckPage({ searchParams }) {
  ...
  const visibleDecks = activeStage
    ? decks.filter(deck => deck.stageSlug === activeStage)
    : decks;
        ...
        <header className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-base" style={{ color: "var(--secondary-label)" }}>
              Download anonymised decks used by Indian founders at seed, Series A, and beyond. Each deck
              is sourced from public accelerators or founder submissions. Use responsibly.
            </p>
          </div>
            ...
            {visibleDecks.map(deck => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <h2 className="text-lg font-semibold leading-tight" style={{ color: "var(--label)" }}>
                    {deck.title}
                  </h2>
                  {deck.stage && (
                    <span
