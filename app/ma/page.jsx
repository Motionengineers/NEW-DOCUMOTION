    ...
    <div className="min-h-screen bg-[var(--system-background)]">
      ...
      <main className="container mx-auto max-w-6xl px-4 py-12 space-y-12">
        <header className="space-y-4 text-center sm:text-left">
          <span
            className="inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-1 text-xs uppercase tracking-[0.3em]"
          ...
          <p className="text-lg sm:text-xl max-w-3xl" style={{ color: 'var(--secondary-label)' }}>
            Buy, sell, merge, and scale startups faster with verified listings, concierge sourcing,
            and bank-grade deal infrastructure— all inside Documotion.
          </p>
        </header>
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          {CORE_SECTIONS.map(section => (
            <article
              key={section.title}
              className="glass rounded-2xl border border-white/10 p-6 space-y-3"
            >
        ...
        <section className="rounded-2xl border border-white/10 bg-blue-600/10 p-8 backdrop-blur-lg space-y-4">
          ...
          <p className="text-base" style={{ color: 'var(--secondary-label)' }}>
            Request your tailored M&amp;A playbook and we’ll respond within one business day with a
            custom strategy, recommended shortlist, and analyst support plan.
          </p>
          <div
            className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 text-sm"
            style={{ color: 'var(--secondary-label)' }}
          >
            <div>
